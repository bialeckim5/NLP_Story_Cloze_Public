...
#reads in preprocessed vecs for embeddings
with open('x_test_em', 'rb') as f:
    x_test_em = np.load(f)
with open('x_train', 'rb') as f:
    x_train_em = np.load(f)
...
train_df = pandas.concat([train_df, x_train_em], axis=1)
test_df = pandas.concat([test_df, x_test_em], axis=1)
